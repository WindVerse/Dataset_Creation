...
for i in range (1, num_of_iterations+1):
    for j in range (0, frames_per_iteration):
        ...
        if target == "displacements":
            first = np.load(dataset_path + f'/flag_{i:04d}_{j:04d}.npy')
            second = np.load(dataset_path + f'/flag_{i:04d}_{j+1:04d}.npy')
            displacement = second[:,:3] - first[:,:3]
            np.save(target_path + f'/target_{i:04d}_{j:04d}.npy', displacement)
            
        elif target == "velocity_differences":
            first = np.load(dataset_path + f'/flag_{i:04d}_{j:04d}.npy')
            second = np.load(dataset_path + f'/flag_{i:04d}_{j+1:04d}.npy')
            velocity_difference = second[:,3:] - first[:,3:]
            np.save(target_path + f'/target_{i:04d}_{j:04d}.npy', velocity_difference)
            
        elif target == "accelerations":
            first = np.load(dataset_path + f'/flag_{i:04d}_{j:04d}.npy')
            second = np.load(dataset_path + f'/flag_{i:04d}_{j+1:04d}.npy')
            acceleration = (second[:,3:] - first[:,3:]) / delta_t
            np.save(target_path + f'/target_{i:04d}_{j:04d}.npy', acceleration)
            
        elif target == "acc_new":
            if j == 0:
                curr = np.load(dataset_path + f'/flag_{i:04d}_{j:04d}.npy')
                acc_new = np.zeros_like(curr[:, :3])
            else:
                prev = np.load(dataset_path + f'/flag_{i:04d}_{j-1:04d}.npy')
                curr = np.load(dataset_path + f'/flag_{i:04d}_{j:04d}.npy')
                next_f = np.load(dataset_path + f'/flag_{i:04d}_{j+1:04d}.npy')
                
                acc_new = (next_f[:,:3] - 2 * curr[:,:3] + prev[:,:3]) / (0.5 * (delta_t ** 2))
                
            np.save(target_path + f'/target_{i:04d}_{j:04d}.npy', acc_new)
            ...
            np.save(target_path + f'/target_{i:04d}_{j:04d}.npy', acc)
